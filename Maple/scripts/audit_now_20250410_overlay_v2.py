...
R=Path(__file__).resolve().parents[1];O=R/'data/now_20250410_overlay_v2';O.mkdir(exist_ok=True)
...
for x in range(368,1263,3):
    ...
    for n,c,(lo,hi) in zip(names,palette,[(305,745),(650,799),(710,823),(705,823),(814,829)]):
        ...
        if n=='field':
            dist[(patch[:,:,0]<205)|(patch[:,:,1]<110)|(patch[:,:,2]<=patch[:,:,1])]=1e6
            # The pale anti-aliased edge of the boss line also matches pink.
            # Field is visibly separate and lower throughout this image.
            dist[np.arange(lo,hi+1)<=v['boss_y']+20,:]=1e6
        if n=='azmoth':
            dist[(patch.mean(2)<165)|(patch[:,:,2]-patch[:,:,0]<8)]=1e6
            # Image-only guide through the smooth light-cyan line, independent of canonical values.
            guide=np.interp(x,[1085,1095,1110,1140,1170,1200,1230,1262],[756,775,779,762,746,733,724,717])
            dist[abs(np.arange(lo,hi+1)-guide)>10,:]=1e6
...
manifest['revision']='v2 excludes boss-edge false field detections; light coin follows manually observed image-only guide; no canonical curve fitting'
...
fig.savefig(R/'figures/total_20250410_overlay_v2.png')
...
for n in names:
    active=d[n+'_px']>0
    qa.scatter(d.loc[active,'x_pixel'],d.loc[active,n+'_y'],s=3,label=n)
qa.set_xlim(320,1300);qa.set_ylim(860,270);qa.legend();fig2.savefig(R/'figures/now_20250410_pixel_qa_v2.png',dpi=150)
...
assert (d.field_y>d.boss_y+20).all()
prior=pd.read_csv(R/'data/now_20250410_overlay/digitized_overlay.csv')
changes=d[['x_pixel','date','field_y','azmoth_y','read_total']].merge(prior[['x_pixel','field_y','azmoth_y','read_total']],on='x_pixel',suffixes=('_v2','_v1'))
changes.to_csv(O/'before_after.csv',index=False)
print('Changed field samples:',int((changes.field_y_v1!=changes.field_y_v2).sum()))
print('Changed cyan samples:',int((changes.azmoth_y_v1!=changes.azmoth_y_v2).sum()))
