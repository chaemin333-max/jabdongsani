...
parser=argparse.ArgumentParser();parser.add_argument('--reward-adjusted',action='store_true');args=parser.parse_args()
version='tier_boss_decomposition_reward_adjusted' if args.reward_adjusted else 'tier_boss_decomposition_v1'
with (ROOT/f'Maple/data/{version}/weekly_decomposition.csv').open(encoding='utf-8') as f:r=list(csv.DictReader(f))
...
fig.text(.5,.92,('신규 보상 기회 보정 · ' if args.reward_adjusted else '')+'리프 전 본섭 하위 생산 일정 + 시즌별 챌섭 상한 가정 · 2026.08.20까지',ha='center',color='#526773')
...
p=ROOT/f'Maple/figures/{version}.png';fig.savefig(p);print(p)
